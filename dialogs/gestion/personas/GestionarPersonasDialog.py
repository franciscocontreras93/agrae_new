...
class GestionarPersonasDialog(QDialog):
    ...
    PersonaSignal = pyqtSignal(object)
    ...
    def _setup_ui(self):
        ...
        self.search_table = PersonaSearchTable(self)
        ...
        self.btn_reload.setToolTip("Recargar lista de personas")
        ...
        self.btn_delete.setToolTip("Eliminar persona seleccionada")
    ...
    def _delete_selected(self):
        item = self.search_table.selected_item()
        if not item:
            QMessageBox.information(self, "aGrae", "Selecciona una persona.")
            return
        ...
        resp = QMessageBox.question(
            self,
            "Eliminar persona",
            f"¿Quieres eliminar a {nombre or 'esta persona'}?"
        )
        if resp != QMessageBox.Yes:
            return

        try:
            r = self.api.delete(f"gis/personas/{idpersona}/")
            ...
            QMessageBox.information(
                self,
                "aGrae",
                f"Persona {nombre or idpersona} eliminada."
            )
            ...
            QMessageBox.warning(
                self,
                "aGrae",
                f"No se pudo eliminar la persona {nombre or idpersona}.\nError: {str(e)}"
            )
